...
Knowing what I know: perfect nim strategy can be determined by converting
the number of stones in each pile to binary and summing the binary numbers 
as though they were integers. If the sum of every individual digit is even,
then the current player will lose.
"""
...
def unpredictedRange(n):
	return range(2**n+1,int(4.0/3.0*float(2**n))+1)

def ibin(n):
	return int(bin(n)[2:])

def printGame(r):
	print "%d %d %d %d %s" % (r,ibin(r),ibin(2*r),ibin(3*r),str(nim(r)))